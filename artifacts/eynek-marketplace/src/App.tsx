  ...
  return (
    <>
      <main className="home-page">
        <section className="hero"><div className="container hero-panel"><div className="hero-content"><div className="hero-kicker">YENİ BAXIŞ</div><h1>Sənə yaraşan çərçivəni tap.</h1><p className="hero-sub">Azərbaycandakı optikaların modellərini bir yerdə kəşf et. Bəyəndiyini seç, almazdan əvvəl görünüşünü önizlə.</p><div className="hero-actions"><Link href="/collection" className="btn hero-cta" data-testid="link-hero-collection">Eynəkləri kəşf et <ArrowRight size={16} /></Link><button className="hero-preview-link" onClick={() => onTryOn(products[0])} data-testid="button-hero-tryon"><ScanFace size={16} /> VTO önizləmə</button></div><div className="hero-note"><Info size={14} /> Nümunə inventar · virtual sınaq demo görünüşüdür</div></div><div className="hero-stats" aria-label="EYNƏK haqqında"><div><strong>{products.length}</strong><span>Seçilmiş model</span></div><div><strong>{vendors.length}</strong><span>Optika</span></div><div><strong>2</strong><span>Kateqoriya</span></div><div><strong>AZ</strong><span>Yerli seçim</span></div></div></div></section>
        <section className="section home-discover"><div className="container"><div className="section-head"><div><div className="home-overline">Sənin üçün seçildi</div><h2 className="section-title">Eynəkləri kəşf et</h2></div><Link href="/collection" className="home-count" data-testid="link-view-all">{products.length} model <ArrowRight size={14} /></Link></div><div className="home-filter-pills" aria-label="Kateqoriyalar"><Link href="/collection" className="selected">Hamısı</Link><Link href="/collection?type=sunglasses">Gün eynəyi</Link><Link href="/collection?type=optical">Optik çərçivə</Link><Link href="/brands">Brendlər</Link></div><div className="product-grid">{products.slice(0, 4).map((product) => <ProductCard key={product.id} product={product} liked={likedIds.has(product.id)} onFavorite={onFavorite} onQuickView={onQuickView} onTryOn={onTryOn} />)}</div></div></section>
