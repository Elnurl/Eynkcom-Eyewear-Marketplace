...
export default function OrderPage() {
  ...
  const order = useGetGuestOrder(orderId, {
    query: { enabled: Boolean(orderId && accessToken), queryKey, retry: false },
  ...
  return <main className="commerce-page"><div className="commerce-wrap">
    <header className="commerce-header"><div><Link href="/" className="brand" data-testid="link-order-brand"><BrandLogo /></Link><div className="eyebrow" style={{ marginTop: 25 }}>Qonaq sifariş izləmə</div><h1>Sifarişin haradadır?</h1></div><p>Sifarişiniz bir neçə optik mağazadan ibarətdirsə, hər mağazanın hazırlıq mərhələsini ayrıca görə bilərsiniz.</p></header>
    {order.isLoading ? <LoadingCard lines={6} /> : order.isError || !order.data ? <QueryError message="Sifariş tapılmadı və ya giriş kodu düzgün deyil." onRetry={() => void order.refetch()} /> : (
      <>
        <section className="commerce-card order-hero"><div><div className="order-number">SİFARİŞ {order.data.orderNumber}</div><h1>{orderStatusLabel(order.data.status)}</h1><p>Yaradılıb: {dateLabel(order.data.createdAt)} · {order.data.deliveryArea}, Bakı</p></div><StatePill status={order.data.status} label={orderStatusLabel(order.data.status)} /></section>
        ...
        <div className="commerce-grid" style={{ marginTop: 16 }}>
          ...
          <aside>
            <section className="commerce-card commerce-card-pad"><div className="commerce-card-heading"><div><h2>Məlumatlar</h2><p>Qonaq sifarişi</p></div><ShieldCheck size={19} color="#5b719a" /></div><dl className="sidebar-summary" style={{ padding: '18px 0 0' }}><div><dt>Müştəri</dt><dd>{order.data.customerName}</dd></div><div><dt>Telefon</dt><dd>{order.data.customerPhone}</dd></div><div><dt>Ərazi</dt><dd>{order.data.deliveryArea}</dd></div><div><dt>Ünvan</dt><dd style={{ textAlign: 'right', maxWidth: 170 }}>{order.data.deliveryAddress}</dd></div><div><dt>Ödəniş</dt><dd>{paymentStatusLabel(order.data.paymentStatus)}</dd></div></dl></section>
