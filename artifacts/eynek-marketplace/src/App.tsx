...
const products: Product[] = [
  { id: 1, name: 'Mimoza 02', vendor: 'Optika Nərgiz', vendorSlug: 'optika-nergiz', price: 118, type: 'Optik çərçivə', gender: 'Qadın', shape: 'Cat-Eye', material: 'Asetat', size: 'M (52–18)', color: 'Kərpic / şampan', tone: 'tone-1', frameStyle: 'coral', description: 'Yumşaq cat-eye xətti və isti kərpic tonu ilə gündəlik görünüşə sakit xarakter verir.' },
  { id: 2, name: 'Sahil 11', vendor: 'Bakı Optik', vendorSlug: 'baki-optik', price: 145, type: 'Gün eynəyi', gender: 'Uniseks', shape: 'Wayfarer', material: 'Asetat', size: 'L (55–19)', color: 'Zeytun', tone: 'tone-2', frameStyle: 'olive', description: 'Günəşli Bakı günləri üçün dərin zeytun asetat və polarizə linza.' },
  ...
  { id: 4, name: 'Xəzər Air', vendor: 'Dəniz Optika', vendorSlug: 'deniz-optika', price: 164, type: 'Gün eynəyi', gender: 'Kişi', shape: 'Aviator', material: 'Titanium', size: 'L (58–16)', color: 'Qızılı / yaşıl', tone: 'tone-4', frameStyle: 'gold', description: 'Titanium aviator quruluşu ilə yüngül və davamlı sahil üslubu.' },
  ...
  { id: 8, name: 'Caspian Sun', vendor: 'Dəniz Optika', vendorSlug: 'deniz-optika', price: 151, type: 'Gün eynəyi', gender: 'Qadın', shape: 'Cat-Eye', material: 'Bio-nylon', size: 'M (53–17)', color: 'Şəffaf bal', tone: 'tone-4', frameStyle: 'coral', description: 'Bal rəngli şəffaf bio-nylon, qaldırılmış künclər və yumşaq linza tonu.' },
];
...
function QuickView({ product, onClose, onAdd, onTryOn }: { product: Product; onClose: () => void; onAdd: (product: Product, message?: string) => void; onTryOn: (product: Product) => void }) {
  return <div className="tryon-backdrop" role="dialog" aria-modal="true" aria-label="Sürətli məhsul baxışı" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="tryon-modal quick-view-modal"><div className="tryon-top" style={{ borderBottom: '1px solid var(--line)', color: 'var(--navy)' }}><div><div className="eyebrow">{product.type}</div><h2>{product.name}</h2><p style={{ color: 'var(--muted)' }}>{product.vendor} · Bakı</p></div><button className="icon-button" onClick={onClose} aria-label="Baxışı bağla" data-testid="button-close-quick-view"><X size={19} /></button></div><div className="quick-view-layout"><div className={`detail-art quick-view-art ${product.tone}`}><FrameVisual shape={product.shape} frameStyle={product.frameStyle} sunglasses={product.type === 'Gün eynəyi'} /></div><div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}><div className="detail-price" style={{ marginTop: 0 }}>{money(product.price)}</div><p className="detail-copy">{product.description}</p><div className="spec-list" style={{ margin: '8px 0 17px' }}><div><dt>Forma</dt><dd>{product.shape}</dd></div><div><dt>Ölçü</dt><dd>{product.size}</dd></div></div><button className="btn btn-full" onClick={() => onAdd(product)} data-testid="button-add-cart-quick-view"><ShoppingBag size={14} /> Səbətə əlavə et</button><button className="try-link" onClick={() => onTryOn(product)} data-testid="button-try-on-quick-view"><ScanFace size={14} /> Üzümdə yoxla</button></div></div></div></div>;
}
