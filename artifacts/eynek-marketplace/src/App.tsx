    ...
    <article className="product-card" data-testid={`card-product-${product.id}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className={`product-image ${product.tone}`}>
        ...
        <div className="product-card-actions">
          ...
          <button className="try-card" onClick={() => onTryOn(product)} data-testid={`button-try-on-${product.id}`}><ScanFace size={17} strokeWidth={2} /> Yoxla</button>
        </div>
      </div>
...
function Home({ products, onQuickView, likedIds, onFavorite, onTryOn }: CommonProps) {
  return (
    <>
      <main>
        <section className="hero"><div className="container hero-grid"><div><div className="eyebrow">EYNƏK marketplace · Azərbaycan</div><h1>Üzünə uyğun eynəyi tap.</h1><p className="hero-sub">Azərbaycandakı müxtəlif optikaların və satıcıların eynəklərini bir yerdə kəşf et. Almazdan əvvəl ölçünü və modeli müqayisə et.</p><div className="hero-actions"><Link href="/collection" className="btn" data-testid="link-hero-collection">Eynəklərə bax <ArrowRight size={15} /></Link><button className="btn btn-secondary" onClick={() => onTryOn(products[0])} data-testid="button-hero-tryon"><ScanFace size={15} /> Yoxla</button></div><div className="hero-note"><Info size={15} /> Nümunə inventar: inkişaf datası</div></div><div className="hero-art" aria-label="Eynək məhsulu üçün redaksiya vizualı"><div className="hero-art-back" /><div className="hero-art-photo" /><div className="hero-glasses"><span className="hero-lens" /><span className="hero-bridge" /><span className="hero-lens" /></div><div className="art-sticker">Bütün<br />eynəklər.<br />Bir marketplace.</div><div className="hero-caption">01 / EYNƏK seçimləri</div></div></div></section>
